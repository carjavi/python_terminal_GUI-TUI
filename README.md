...
├── 1.1  Identidad visual        → Cambiar de texto de las ventanas
├── 1.2  Funciones del menú      → 4 funciones autocontenidas (Python requiere que las funciones estén definidas antes del bloque Menu)
│   ├── option_1()               → 
│   ├── option_2()               → 
│   │                               
│   ├── option_3()               → 
│   └── option_4()               → 
└── 1.3  Mapa del menú           → MENU_ITEMS 

SECCIÓN 2 — MOTOR DE INTERFAZ
```

Estructura general de la función para Ventana modal y Barra de progreso
```bash
def mi_funcion(
    manager: ptg.WindowManager,        # regla 1
    set_progress: Callable[[float], None],  # regla 2
) -> None:
```

```bash
def option_1(manager, set_progress): # siempre espera 2 parametros
    # Configuración
    carpeta_destino = "C:/mis_archivos"

    # Lógica — puede ser cualquier cosa
    set_progress(0.5)
    os.makedirs(carpeta_destino, exist_ok=True)
    _show_message(manager, "Listo", f"Carpeta creada:\n{carpeta_destino}")
```
...
# Troubleshooting
> :warning: **Warning:** 
>Al iniciar por primera vez instala los driver requeridos, pero puede que necesite cerrarse y abrir de nuevo para funcionar
